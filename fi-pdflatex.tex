...
63\,\% of people worldwide can utilize it to convey or acquire new information.
With the increased usage also comes an increase in available resources. While
this is great for variety, it also sometimes creates an obstacle for anyone
searching for valuable learning materials. It can be hard to find to filter
helpful from poorly written or incorrect.
...
such as a glossary on academic integrity or a database of educational materials.
...
However, even if WordPress has many templates, many pages may look identical.
The identical look is also true for the administration user interface, which is
not entirely wrong, but occasionally can be too constrained or complex for
specific tasks.
...
The work is to replace the existing WordPress plugin with a custom PHP solution.
In order to do so, several steps were taken. First, the plugin was examined
for the possibility of utilizing its source code, be it wholly or in parts, inside
the new application. It has been deemed to be too profoundly interconnected with
WordPress and therefore discarded. Instead, a requirements and design analysis
process was executed and specified in section~\ref{sect:plugin}.

Further research was conducted in the field of available open-source solutions.
While there is a multitude of PHP web applications that handle database records
and their presentation, none meet the established criteria for the handling of
